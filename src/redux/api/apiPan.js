...
import {getPanFail,getPanStart,getPanSuccess,updatePanFail,updatePanStart,updatePanSuccess, updateRGBFail, updateRGBStart, updateRGBSuccess} from '../panSice'

export const updateManually = async (id, dispatch, data,navigate) => {
    dispatch(updatePanStart());
    try {
      const res = await axios.put(
        `${process.env.REACT_APP_BACKEND_URL}api/devices/manually/${id}`, data);
...
export const updateRGB= async(dispatch,id,data) => {
  ...
  try{
      console.log(process.env.REACT_APP_BACKEND_URL);
      const res = await axios.put(
        `${process.env.REACT_APP_BACKEND_URL}api/devices/rgb/${id}`, data);
